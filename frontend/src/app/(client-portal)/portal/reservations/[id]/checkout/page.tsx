...
async function fetchReservation(id: string): Promise<Reservation> {
  const { data } = await apiClient.get(`/properties/reservations/mine/${id}/`);
  return data;
}
...
export default function ReservationCheckoutPage() {
  ...
  const { data: reservation, isLoading, isError } = useQuery({
    queryKey: ["reservation-checkout", params.id],
  ...
  async function handlePayPal(popup: Window | null) {
    ...
    setError(null);
    try {
      const returnUrl = `${window.location.origin}/portal/reservations/${reservation.id}/checkout/paypal-return`;
      const cancelUrl = `${window.location.origin}/portal/reservations/${reservation.id}/checkout?cancelled=1`;
  ...
  async function handlePaymongo(type: PaymongoMethodType, popup: Window | null) {
    ...
    try {
      ...
      const returnUrl = `${window.location.origin}/portal/reservations/${reservation.id}/checkout/paymongo-return`;
      const card =
  ...
  if (isError || !reservation) {
    return (
      <div className="max-w-md">
        <p className="text-red-700">Couldn&apos;t load this reservation.</p>
      </div>
    );
  }
  ...
  if (reservation.status !== "pending_payment") {
    return (
      <div className="max-w-md">
        <p className="text-neutral-600">This reservation is no longer awaiting a fee payment.</p>
      </div>
    );
  }

  const lot = reservation.lot_detail;

  return (
    <div>
      <h1 className="font-display font-semibold uppercase text-2xl mb-1 text-ink">Checkout</h1>
      <p className="text-neutral-600 text-sm mb-8">Pay your reservation fee to confirm your hold on this lot.</p>
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-8">
        ...
        <div className="border border-divider p-6 h-fit lg:sticky lg:top-6">
          ...
          <h2 className="font-display font-semibold uppercase text-lg text-ink mb-1">{reservation.lot_display}</h2>
          {lot?.project_name && <p className="text-neutral-600 text-sm mb-3">{lot.project_name}</p>}
          {lot?.area_sqm && <p className="text-neutral-600 text-sm mb-1">{lot.area_sqm} sqm</p>}
          {lot?.total_price && (
          ...
          <div className="border-t border-divider pt-4 mb-4">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-ink font-semibold font-data">
                {currency}{Number(reservation.reservation_fee).toLocaleString()}
              </span>
            </div>
            <p className="text-neutral-500 text-xs">Due by {reservation.deadline}</p>
          </div>
