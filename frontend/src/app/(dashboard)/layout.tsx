...
const STAFF_ROLES = ["admin", "sales_agent", "accountant"];
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!stored || !STAFF_ROLES.includes(stored.role)) {
  ...
  const roleLabel =
    user.role === "admin" ? "Admin" : user.role === "sales_agent" ? "Sales Agent" : "Accountant";

  return (
    <div className="flex-1 flex">
      <aside className="w-64 shrink-0 border-r border-stone-200 bg-white px-4 py-8 hidden sm:block">
        <p className="text-xs uppercase tracking-wide text-stone-400 px-3 mb-4">{roleLabel}</p>
      ...
      <div className="flex-1 px-6 sm:px-10 py-10 max-w-5xl">{children}</div>
    </div>
  );
}